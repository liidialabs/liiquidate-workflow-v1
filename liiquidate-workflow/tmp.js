...
var upsertPositionData = (dataToSend, secretKey) => (sendRequester) => {
...
function readUserPositionFromSupabase(runtime2, user, chain) {
  const supabaseServiceKey = runtime2.getSecret({ id: "SUPABASE_KEY" }).result();
  const httpClient = new ClientCapability2;
  const result = httpClient.sendRequest(runtime2, readUserPositions(supabaseServiceKey.value, user, chain), consensusIdenticalAggregation())().result();
  runtime2.log(`Successfully read user's position data`);
  return result;
}
...
function liquidatePositions(runtime2, positions, chain) {
  ...
  const evmCfg = runtime2.config.evms[chain];
  const network248 = getNetwork({
    chainFamily: "evm",
  ...
  if (writeReportResult.txStatus === TxStatus.SUCCESS) {
    ...
    runtime2.log(`Fatal error: ${writeReportResult.errorMessage || "Unknown error"}`);
    throw new Error(`Fatal write error: ${writeReportResult.errorMessage}`);
  }
  const txHash = bytesToHex(writeReportResult.txHash ?? new Uint8Array(32));
  return txHash;
}
function getCurrentPosition(runtime2, user, chain) {
  const evmConfig = runtime2.config.evms[chain];
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: evmConfig.chainSelectorName,
...
var chainlinkPriceOracleEventAbi = parseAbi([
  "event AnswerUpdated(int256 indexed current, uint256 indexed roundId, uint256 updatedAt)"
]);
var onLogTriggerLiidiaV1 = (runtime2, log) => {
  runtime2.log("Running Liidia LogTrigger ...");
  ...
  const SUPPLY_EVENT_MAP = {
    Supply: (args) => ({ user: args.user, token: args.token }),
    Withdraw: (args) => ({ user: args.redeemFrom, token: args.token }),
    Liquidated: (args) => ({ user: args.from, token: args.token })
  };
  const normalizer1 = SUPPLY_EVENT_MAP[decodedLog.eventName];
  if (normalizer1) {
    const { user, token } = normalizer1(decodedLog.args);
    runtime2.log(`>>> Decoding ${decodedLog.eventName} Event <<<`);
    runtime2.log(`- User: ${user}`);
    runtime2.log(`- Collateral: ${token}`);
    const { hf, status } = getRiskStatus(runtime2, user, chain);
    runtime2.log(`- Health Factor: ${hf}`);
    runtime2.log(`- Status: ${status}`);
    dataToSend.user = user;
    dataToSend.collateral = token;
    dataToSend.hf = hf;
    dataToSend.status = status;
  }
  const BORROW_EVENT_MAP = {
    Borrow: (args) => args.user,
    RepayUsdc: (args) => args.user
  };
  const normalizer2 = BORROW_EVENT_MAP[decodedLog.eventName];
  if (normalizer2) {
    const user = normalizer2(decodedLog.args);
    runtime2.log(`>>> Decoding ${decodedLog.eventName} Event <<<`);
    runtime2.log(`- User: ${user}`);
    const { collateral } = readUserPositionFromSupabase(runtime2, user, chain);
    const { hf, status } = getRiskStatus(runtime2, user, chain);
    runtime2.log(`- Health Factor: ${hf}`);
    runtime2.log(`- Status: ${status}`);
    dataToSend.user = user;
    dataToSend.collateral = collateral;
    dataToSend.hf = hf;
    dataToSend.status = status;
  }
  if (dataToSend.status === 0 && BigInt(dataToSend.collateral ?? 0) < BASE_HF) {
    const positionData = {
      ...
      collateral: dataToSend.collateral
    };
    const liquidPositions = checkIfLiquidatable(runtime2, chain, [positionData]);
...
var onLogTriggerOracles = (runtime2, log) => {
  runtime2.log("Running Oracles LogTrigger");
  const topics = log.topics.map((topic) => bytesToHex(topic));
  const data = bytesToHex(log.data);
  const decodedLog = decodeEventLog({
